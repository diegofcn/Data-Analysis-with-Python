...
def calculate(list):
    ...
    # reshape the list into an numpy array
    list = np.array(list).reshape((3, 3))

    # calculations
    mean = [list.mean(axis=0).tolist(), list.mean(axis=1).tolist(), np.mean(list).tolist()]
    var = [list.var(axis=0).tolist(), list.var(axis=1).tolist(), np.var(list).tolist()]
    std = [list.std(axis=0).tolist(), list.std(axis=1).tolist(), np.std(list).tolist()]
    ...
    return {"mean": mean, "variance": var, "standard deviation": std, "max": max, "min": min, "sum": sum}
